...
# ---- load csv file ----
df <- read.csv('/Users/hunglo/Documents/inscopix_csv/opto_csv_chr2/processed_csv/opto_data.csv',header=TRUE)
# df <- df %>% group_by(mouse_id) %>% mutate(m = -mean(milk.consumption)) %>% arrange(desc(m)) %>% select(-m)

# df <- na.omit(df) # remove rows with NA
df_norm <- read.csv('/Users/hunglo/Documents/inscopix_csv/opto_csv_chr2/processed_csv/opto_data_norm.csv',header=TRUE)
...
virus.labs <- c("GFP","ChR2")
names(virus.labs) <- c("0_GFP", "1_ChR2")
...
fig1 <- ggplot(data = df, aes(x= LED.state, y=milk.consumption))+
  ...
  geom_beeswarm(alpha=0.3,size=2,dodge.width = 0.75,cex=4,stroke=NA,)+
  geom_smooth(method='lm',aes(group=mouse_id),linewidth=0.5, linetype = "dashed",position=dodge,se=FALSE, alpha=0.25,color='grey')+
  ...
  scale_y_continuous(name = "# of pump event")+
...
#   scale_color_manual(values=c('black','black','black','black','black','black','black','black'))+ # 
#   
#   scale_y_continuous(name = "# of pump event (normalized)")+
...
#   theme(axis.title.x=element_blank(), legend.position = "none")+
#   scale_color_manual(values=c('black','black','black','black','black','black','black','black'))+ # 
#   scale_y_continuous(name = "# of lick events")+
#   ggtitle("Total licks")
# 
# fig1
...
milk.cont <- contrast(milk.emmn, method="trt.vs.ctrl", ref="LED off 1_ChR2")
milk.cont <- contrast(milk.emmn, method="pairwise")

milk.cont
plot(milk.cont,xlab='Difference in milk consumption') + geom_vline(xintercept = 0,linetype="dashed")
